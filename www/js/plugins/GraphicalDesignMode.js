...
(function () {
  ...
  var settings = {
    ...
    /* メインメニュー画面の追加情報 */
    Scene_Menu: {
      pictures: [{ file: "", switchId: 0 }],
      windows: [{ lines: [], switchId: 0 }],
    },
    /* 戦闘画面の追加情報 */
    Scene_Battle: {
      pictures: [{ file: "", switchId: 0 }],
  ...
  var paramIconSizeScale = getParamBoolean([
    "IconSizeScale",
    "アイコンサイズ調整",
  ]);
  var paramBackgroundFixed = getParamBoolean([
    "BackgroundFixed",
    "背景表示可否固定",
  ]);
  var paramRightClickHide = getParamBoolean([
    "RightClickHide",
    "右クリックで消去",
  ]);
  var paramIgnoreMesWindow = getParamBoolean([
    "IgnoreMesWindow",
    "メッセージウィンドウを無視",
  ]);
  ...
  Utils.isDesignMode = function () {
    return (
      (this.isOptionValid("test") ||
        this.isOptionValid("btest") ||
        this.isOptionValid("etest")) &&
      this.isNwjs() &&
      paramDesignMode
    );
  };
  ...
  if (Utils.isDesignMode()) {
    ...
    SceneManager.updateDragInfo = function () {
      if (Input.isPressed("control") && Input.isTriggered("copy")) {
        ...
        if (this._copyCount % 2 === 0) {
          ...
          this.setInfoExtend(
            "X座標[" + copyValue + "]をクリップボードにコピーしました。",
            0
          );
        } else {
          copyValue = this.lastWindowY.toString();
          this.setInfoExtend(
            "Y座標[" + copyValue + "]をクリップボードにコピーしました。",
            0
          );
        }
    ...
    SceneManager.setInfoExtend = function (value, add) {
      ...
      console.log(
        add ? this.controlNumber + (add < 0 ? 1 : 0) + " : " + value : value
      );
    ...
    DataManager.saveDataFileWp = function () {
      StorageManager.saveToLocalDataFile(
        this._databaseFileCp.src,
        window[this._databaseFileCp.name]
      );
    };
    ...
    Scene_Base.prototype.shiftGridToLast = function () {
      if (
        this._gridSprite &&
        this.children[this.children.length - 1] !== this._gridSprite
      ) {
    ...
    Scene_Base.prototype.updateDrag = function () {
      ...
      var result = this._windowLayer.children
        .clone()
        .reverse()
        .some(function (container) {
          return (
            checkTypeFunction(container.processDesign) &&
            container.processDesign()
          );
        }, this);
      if (result) return;
      this.children
    ...
    Scene_Base.prototype.createWindowLayer = function () {
      if (!(this instanceof Scene_Boot) && !(this instanceof Scene_Map))
    ...
    //  コンテナをドラッグ＆ドロップします。
    //=============================================================================
    var _PIXI_DisplayObjectContainer_initialize =
      PIXI.Container.prototype.initialize;
    ...
    PIXI.Container.prototype.processDesign = function () {
      ...
      if (!this.moveDisable) {
        if (this.processPosition()) {
          ...
          if (!this._holding)
            SceneManager.setInfoExtend("位置を変更しました。" + info, 1);
          result = true;
        }
        ...
        if (this.processInput()) {
          SceneManager.setInfoExtend(
            this._propLabel + "の値を" + this._propValue + "に変更しました。",
            1
          );
          result = true;
        }
    ...
    PIXI.Container.prototype.processPosition = function () {
      ...
      if (
        this.isTouchEvent(TouchInput.isTriggered) ||
        (this._holding && TouchInput.isPressed())
      ) {
      ...
      if (this.isPreparedEvent()) {
        var params = [
          ...
          [
            ...
            1,
            100,
            this.resetFontSettings.bind(this),
            true,
          ],
          [
            "num5",
            ...
            1,
            2000,
            this.setFittingHeight.bind(this),
            true,
          ],
          [
            "num6",
            ...
            0,
            255,
            this.updateBackOpacity.bind(this),
            true,
          ],
          [
            "num7",
          ...
          [
            ...
            null,
            null,
            this.createBackSprite.bind(this),
            true,
          ],
          [
            "num9",
    ...
    Window_Base.prototype.setFittingHeight = function () {
      if (this._customLineNumber)
        this.height = this.fittingHeight(this._customLineNumber);
    };

    Window_Base.prototype.processSetProperty = function (
      keyCode,
      propLabel,
      propName,
      min,
      ...
      if (Input.isTriggered(keyCode)) {
        ...
        if (result || (stringFlg && result === "")) {
          this._windowFrameSprite.setBlendColor([0, 0, 0, 0]);
          SceneManager.pushChangeStack(this);
          this[propName] = stringFlg
            ? getArgString(result)
            : getArgEval(result, min, max);
    ...
    PIXI.Container.prototype.updateSnapX = function (x) {
      ...
      for (var i = 0, n = children.length; i < n; i++) {
        ...
        if (
          child !== this &&
          this.isSameInstance(child) &&
          child.isTouchable() &&
          child.isOverlapY(this)
        ) {
      ...
      if (minDistanceR > Math.abs(Graphics.boxWidth - endX))
        return Graphics.boxWidth - this.width;
    ...
    PIXI.Container.prototype.updateSnapY = function (y) {
      ...
      if (minDistanceD > Math.abs(Graphics.boxHeight - endY))
        return Graphics.boxHeight - this.height;
    ...
    PIXI.Container.prototype.isTouchPosInRect = function () {
      var tx = TouchInput.x;
      var ty = TouchInput.y;
      return tx >= this.x && tx <= this.endX && ty >= this.y && ty <= this.endY;
    };
    ...
    Sprite.prototype.isTransparent = function () {
      ...
      var cos = Math.cos(-this.rotation);
      var bx =
        Math.floor(dx * cos + dy * -sin) / this.scale.x +
        this.anchor.x * this.width;
      var by =
        Math.floor(dx * sin + dy * cos) / this.scale.y +
    ...
    PIXI.Container.prototype.isOverlapX = function (win) {
      return (
        this.isRangeX(win.x) ||
        this.isRangeX(win.endX) ||
        win.isRangeX(this.x) ||
        win.isRangeX(this.endX)
      );
    };
    ...
    PIXI.Container.prototype.isOverlapY = function (win) {
      return (
        this.isRangeY(win.y) ||
        this.isRangeY(win.endY) ||
        win.isRangeY(this.y) ||
        win.isRangeY(this.endY)
      );
    };
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    this.pluginCommandGraphicalDesignMode(
      command.replace(metaTagPrefix, ""),
      args
    );
  };

  Game_Interpreter.prototype.pluginCommandGraphicalDesignMode = function (
    command
  ) {
  ...
  //=============================================================================
  DataManager._databaseFileCp = {
    name: "$dataContainerProperties",
    src: "ContainerProperties.json",
  };
  ...
  DataManager.loadDatabase = function () {
    ...
    this.loadDataFileAllowError(
      this._databaseFileCp.name,
      this._databaseFileCp.src,
      errorMessage
    );
  };
  ...
  DataManager.isDatabaseLoaded = function () {
    return (
      _DataManager_isDatabaseLoaded.apply(this, arguments) &&
      window[this._databaseFileCp.name]
    );
  };
  ...
  PIXI.Container.prototype.saveContainerInfo = function () {
    var sceneName = SceneManager.getSceneName();
    var parentName = getClassName(this.parent);
    if (!$dataContainerProperties[sceneName])
      $dataContainerProperties[sceneName] = {};
    var sceneInfo = $dataContainerProperties[sceneName];
  ...
  Window_Base.prototype.createBackSprite = function () {
    if (this._customBackFileName) {
      ...
      this._customBackSprite.bitmap = ImageManager.loadPicture(
        this._customBackFileName,
        0
      );
    ...
    if (
      Utils.isDesignMode() &&
      this._customBackSprite &&
      this._customBackSprite.bitmap
    ) {
  ...
  Window_Base.prototype.standardFontFace = function () {
    return this._customFontFace
      ? this._customFontFace
      : _Window_Base_standardFontFace.apply(this, arguments);
  };

  var _Window_Base_standardFontSize = Window_Base.prototype.standardFontSize;
  Window_Base.prototype.standardFontSize = function () {
    return this._customFontSize
      ? eval(this._customFontSize)
      : paramFontSize
      ? paramFontSize
      : _Window_Base_standardFontSize.apply(this, arguments);
  };

  var _Window_Base_standardPadding = Window_Base.prototype.standardPadding;
  Window_Base.prototype.standardPadding = function () {
    return this._customPadding
      ? eval(this._customPadding)
      : paramPadding
      ? paramPadding
      : _Window_Base_standardPadding.apply(this, arguments);
  };

  var _Window_Base_lineHeight = Window_Base.prototype.lineHeight;
  Window_Base.prototype.lineHeight = function () {
    return this._customLineHeight
      ? eval(this._customLineHeight)
      : paramLineHeight
      ? paramLineHeight
      : _Window_Base_lineHeight.apply(this, arguments);
  };

  var _Window_Base_standardBackOpacity =
    Window_Base.prototype.standardBackOpacity;
  Window_Base.prototype.standardBackOpacity = function () {
    return this._customBackOpacity
      ? eval(this._customBackOpacity)
      : paramBackOpacity
      ? paramBackOpacity
  ...
  Window_Base.prototype.getIconScale = function () {
    ...
    return paramIconSizeScale && defaultFontSize !== fontSize
  ...
  Window_Custom.prototype.initialize = function (lines) {
    ...
    Window_Selectable.prototype.initialize.call(
      ...
      0,
      320,
      this.fittingHeight(this._lines.length)
    );
    this.refresh();
  };
  ...
  Window_Custom.prototype.changeTextAlign = function (text) {
    ...
    text = text.replace(
      ...
      function () {
        this._textAlign =
          Window_Custom._textAligns[arguments[1].toLowerCase()] || 0;
        return "";
      }.bind(this)
    );
    return text;
