...
(function () {
  ...
  var useBattleInfo =
    String(parameters["Use Battle Info"] || "true") === "true";
  var useRewardsInfo =
    String(parameters["Use Rewards Info"] || "true") === "true";
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (command === "ShowInfo" || command === "インフォ表示") {
      CommonPopupManager.showInfo({}, '\\C[3]Registered\\C[0] ' + args[0].replace(/_/g, ' '), null);
    }
  };
  ...
  Game_Actor.prototype.addParam = function (paramId, value) {
    ...
    if (CommonPopupManager._popEnable) {
      if ($gameParty.inBattle() && !battleShowList.contains("params")) {
        return;
      }
  ...
  Game_Party.prototype.gainGold = function (amount) {
    ...
    if (CommonPopupManager._popEnable) {
      if ($gameParty.inBattle() && !battleShowList.contains("gold")) {
        return;
      }
      var hash = {
        name: "",
  ...
  Game_Actor.prototype.learnSkill = function (skillId) {
    ...
    if (CommonPopupManager._popEnable && !isLearn) {
      if ($gameParty.inBattle() && !battleShowList.contains("skill")) {
        return;
      }
      CommonPopupManager.showInfo(
        $dataSkills[skillId],
  ...
  Game_Actor.prototype.changeExp = function (exp, show) {
    ...
    if (this.level - plevel !== 0) {
      ...
      if (CommonPopupManager._popEnable) {
        if ($gameParty.inBattle() && !battleShowList.contains("level")) {
          return;
        }
        CommonPopupManager.showInfo(
          {
  ...
  if (Imported["VXandAceHybridClass"]) {
    // Change Class Level
    var _gInfo_GInterpreter_changeClassLevel =
      Game_Interpreter.prototype.changeClassLevel;
    Game_Interpreter.prototype.changeClassLevel = function (
      actorId,
      level,
      show
    ) {
    ...
    Game_Actor.prototype.changeAbp = function (abp, show) {
      ...
      if (this.currentClassLevel() - plevel !== 0) {
        ...
        if (CommonPopupManager._popEnable) {
          if ($gameParty.inBattle() && !battleShowList.contains("classLevel")) {
            return;
          }
          CommonPopupManager.showInfo(
            {
  ...
  BattleManager.displayRewards = function () {
    ...
    if (Imported["BattleFormation"]) {
      ...
      if (CommonPopupManager.popEnable() && item) {
        if ($gameParty.inBattle() && !battleShowList.contains("formationLevel"))
          return;
  ...
  CommonPopupManager.showInfo = function (object, value, type, actor, c) {
    ...
    if (c) {
      text1 = text1.replace(/_class/g, c);
    }
    ...
    text1 = descs[0]
      ? text1.replace(/_desc1/g, descs[0])
      : text1.replace(/_desc1/g, "");
    text1 = descs[1]
      ? text1.replace(/_desc2/g, descs[1])
      : text1.replace(/_desc2/g, "");
    ...
    if (!CommonPopupManager._tempCommonSprites)
      CommonPopupManager._tempCommonSprites = [];
    ...
    if ($gameTemp._popupDelay && arg.delay === 0)
      arg.delay += $gameTemp._popupDelay;
  ...
  Window_InfoLog.prototype.drawItem = function (index) {
    ...
    for (var i = 0, max = ts.length; i < max; i++) {
      ...
      if (max === 1 && logRow > 1)
        yy += (this.lineHeight() * logRow) / 2 - this.lineHeight() / 2;
